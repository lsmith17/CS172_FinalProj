...
int main(){
	...
	if (choice == 1){
		ifstream input;
		input.open("Pythagoreanlesson.txt"); // Opens up the Pythagorean Lesson
		...
		string lesson;
		while (!input.eof()){ // Couts every character while there is still strings to pull out of the file
			getline(input, lesson);
			cout << lesson << endl;
		}
		input.close();
	...
	if (choice == 1){
		ifstream input;
		input.open("Rootslesson.txt"); // Opens up the Roots Lesson
